...
本次更新增加外部报表目录/XML 导入路径记忆与连接成功自动保存。首次导入后，下次打开自动从原路径读取；「连接并保存」成功后自动保存数据库配置，默认加密保存密码，可取消勾选。保留全部报表加搜索栏、位置配置、关联文件检查、Excel 导出及原有适配规则。真实 Oracle 执行、数值结果及选项交互尚未在本机验证，不能据此声称所有报表都已正确查询。
...
旧版本没有保存来源，所以升级后需要导入一次外部报表目录或 XML，此后自动恢复。`import-sources.json` 只记住路径和类型，原 XML 必须仍可访问；路径失效会提示并保留记录，恢复路径后重启即可。左侧 ↻ 从原 XML 刷新当前清单，不保存查询参数或结果。已适配的表格查询仍按原定义执行；数据不复刻打印版式公式。

外部位置继续区分已确认入口、同名候选和停用菜单。连接成功后配置单独保存到 `%LOCALAPPDATA%\ReportDesk\connection.json`；密码仅在勾选保存时用 CurrentUser DPAPI 加密，默认勾选，取消选择会记住。连接失败、取消或保存失败不会覆盖原配置。「保存设置」仍可不连接直接保存。升级兼容读取旧 catalog 的连接设置，不恢复其中报表，也不改写或删除旧 catalog。
...
左侧文件夹图标 → 选择报表根目录或报表目录。程序递归识别查询 XML 和 FarPoint 版式 XML，显示导入/匹配摘要；其他 XML 不作为报表导入。扫描警告不会静默丢弃，取消不保存本次半成品。
...
匹配信息按需读取原文件，不复制或执行版式文件。应保持源目录可访问；当前会话记住所选扫描根目录，重启后清单及扫描根目录均清空，重新选择报表目录或 XML 即可。
