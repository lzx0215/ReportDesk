...
task('正在启动…',async()=>{const boot=await call('bootstrap');reports=boot.reports;settings=boot.settings;$('#demo').hidden=!boot.demoVisible;$('#connection-state').textContent=boot.offline?'离线验证模式':'尚未连接数据库';renderList();await select(filteredReports()[0]?.id??null);progress('准备就绪。');});
