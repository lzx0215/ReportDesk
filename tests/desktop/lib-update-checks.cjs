// Offline update/rollback + actual LIB import. No invented report/result data.
...
const run=path.join(root,'artifacts/verification/desktop','lib-update-'+Date.now()),installed=path.join(run,'installation with spaces');
...
(async()=>{
 ...
 const imported=await bridge.call('import',{path:process.argv[4]||'D:/系统知识库/00_Inbox/yljhis/LIB/LIB',folder:true});assert.equal(imported.errors.length,0);assert.equal(imported.reports.length,1284);assert.equal(imported.incomplete.length,68);
 assert.equal(imported.reports.filter(r=>r.issues.length>0).length,22);
 ...
 fs.writeFileSync(path.join(run,'PASS.txt'),'PASS exact baseline verification; three-file apply, idempotence, packaged Host actual LIB 1284 reports/22 blocked/68 incomplete, all report details, rollback/reapply and dependency hashes. No Oracle or fabricated report/result data.\n');
 fs.writeFileSync(path.join(run,'result.json'),JSON.stringify({installed,update,baseline,reports:1284,pending:22,incomplete:68,oracle:'NOT RUN'},null,2));console.log('PASS '+run);
