...
(async () => {
  ...
  boot = await current.call('bootstrap'); assert.equal(boot.reports.length, 1); assert.equal(boot.settings.name, 'new connection'); assert.equal(boot.settings.hasPassword, true);
  assert.equal((await current.call('select', { reportId: id })).id, id);
  ...
  await stop(current); current = start(fresh); await current.ready; assert.equal((await current.call('bootstrap')).reports.length, 1);
  await stop(current); current = null;
  fs.writeFileSync(path.join(run, 'PASS.txt'), 'PASS: import/recheck/query/restart from remembered paths; definitions/results not persisted; legacy connection-only fallback and untouched old catalog; independent connection settings and opt-out password persistence; retired endpoints rejected. No Oracle connection.\n');
