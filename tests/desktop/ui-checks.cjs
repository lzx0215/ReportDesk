...
(async()=>{
 ...
 assert.equal(await page.locator('tbody tr').count(),0);assert.ok(!fs.readFileSync(path.join(data,'connection.json'),'utf8').includes('UI_PRIVATE_PASSWORD'));assert.ok(!fs.existsSync(path.join(data,'catalog.json')));
 ...
 assert.ok((await page.locator('#notice-body').textContent()).includes('匹配到配套文件不代表'));await page.click('[data-close=notice]');
 ...
 assert.equal(await page.locator('#progress').evaluate(n=>getComputedStyle(n).height),'10px');
 assert.deepEqual(errors,[]);fs.writeFileSync(path.join(dir,'PASS.txt'),'PASS: actual Electron + .NET backend; query/filter/sort/copy/export; read-only report info; no favorites/recent/categories; settings/DPAPI without catalog; original progress height; no renderer errors.\n');console.log('PASS desktop UI: '+dir);
